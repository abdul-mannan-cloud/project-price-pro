  ...
  try {
    ...
    const analysisResponse = await fetch('https://api.llama-api.com/chat/completions', {
      ...
      body: JSON.stringify({
        messages: [{
          role: 'system',
          content: 'You are a construction project analyzer. Generate questions to understand project requirements.'
        }, {
          role: 'user',
    ...
    const questions = analysisData.choices[0].message.content.split('\n')
