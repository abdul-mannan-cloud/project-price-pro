...
function formatEstimateCategories(
  categories: Array<{ category: string; amount: string }>,
): string {
  if (!categories || categories.length === 0) return "";

  return categories.map((cat) => `${cat.category}: ${cat.amount}`).join("\n");
}
...
function replacePlaceholders(
  ...
  const replacements: Record<string, string> = {
    ...
    "{availableAppointments}": formatAvailableAppointments(
      data.availableAppointments || "",
    ),
    "{estimateCategories}": formatEstimateCategories(
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!TELNYX_API_KEY || !FROM_PHONE_NUMBER) {
      ...
      throw new Error(
        "TELNYX_API_KEY or TELNYX_PHONE_NUMBER is not configured in environment variables",
      );
    }
    ...
    if (!telnyxResponse.ok) {
      ...
      throw new Error(
        `Telnyx API error: ${telnyxResponse.status} ${telnyxResponse.statusText} - ${JSON.stringify(responseData)}`,
      );
    }
