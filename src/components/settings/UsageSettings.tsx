...
export const UsageSettings = ({ contractor }) => {
  ...
  const fetchPaymentMethods = async () => {
    try {
      ...
      setCards(
        response.data.paymentMethods.data.map((method) => {
        ...
        <div className="border rounded-md overflow-hidden">
          {cards.length > 0 ? (
            cards.map((card, index) => (
              <div
                key={index}
                className={`flex items-center justify-between p-4 border-b border-gray-200 last:border-b-0 ${card.isDefault ? "bg-blue-50" : ""}`}
              >
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {!card.isDefault && cards.length > 1 && (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          removePaymentMethod(card.paymentMethodID)
                        }
                        disabled={
                          removingPaymentMethod === card.paymentMethodID
                        }
      ...
      <div className="border rounded-md p-4 bg-gray-50">
        <div className="flex justify-between items-center">
          ...
          <span className="font-semibold">
            ${formatCashCredits(contractor?.cash_credits || 0)}
          </span>
        </div>
      </div>
