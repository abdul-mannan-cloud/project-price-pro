...
const stripePromise = loadStripe(
  "pk_test_51R7hjJGwj3ICel7hM1235wRDn3lEBEvmYkURzopLYmPpyQa91vdv6HTHffkG5EZFlJBD8v2ruWEhDCknbG8XJn3B00jVdJp7dy",
);
...
function PaymentForm({
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (confirmResult.error) {
        throw new Error(
          confirmResult.error.message || "Failed to confirm card setup",
        );
      }
      ...
      if (attachError) {
        throw new Error(
          attachError.message || "Failed to attach payment method to customer",
        );
      }
  ...
  return (
    <form onSubmit={handleFormSubmit} className="space-y-4">
      {!setIsPaymentModalOpen && (
        <>
          ...
          <p className="text-[15px] text-[#86868b]">
            No charges will be made at this time. Adding a card helps us verify
      ...
      {success && (
        <p className="text-green-500">Payment method saved successfully!</p>
      )}
      {!setIsPaymentModalOpen && (
        <div className="mt-6 text-xs text-center text-gray-500 flex flex-row gap-2 items-center align-middle p-2 border rounded-md bg-gray-50">
          ...
          <span>
            All payment details are encrypted and securely processed via Stripe
      ...
      <div
        className={`flex ${isSettingsPage ? "justify-between" : "justify-between"} pt-6`}
      >
        <Button
          variant="ghost"
          onClick={() => handleBack()}
        ...
        <Button
          type="submit"
          disabled={
            !stripe || loading || isProcessing || !isFormComplete || success
          }
