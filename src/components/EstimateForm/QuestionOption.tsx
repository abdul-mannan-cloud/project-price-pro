...
export const QuestionOption = ({
    ...
    if (isSelected && hasSpecialInput) {
      setTimeout(() => {
        if (option.type === "text_input") {
          if (
            option.character_limit &&
            parseInt(option.character_limit) > 100
          ) {
  ...
  const renderInputField = () => {
    ...
    if (option.type === "text_input") {
      const characterLimit = option.character_limit
        ? parseInt(option.character_limit)
        : undefined;
    ...
    <div
      className={cn(
        ...
        hasSpecialInput && isSelected
          ? "shadow-sm border-2 border-primary "
          : "",
      )}
    >
      <div
        ...
        {showImage && option.image_url && (
          <div className="h-24 w-full flex justify-end aspect-square overflow-hidden">
            <img
              ...
              alt={option.label}
              className="min-w-32 max-w-32 h-full object-cover transition-transform duration-300 group-hover:scale-105"
            />
          </div>
        )}
      </div>
