...
export function CameraMeasurementModal({
  ...
  const [step, setStep] = useState<
    "upload" | "describe" | "measuring" | "results"
  >("upload");
  ...
  const startMeasuring = async () => {
    ...
    if (description.trim().length > 220) {
      toast({
        ...
        description:
          "Please provide a less detailed description (maximum 220 characters).",
        variant: "destructive",
      });
      return;
    }
                ...
                {imagePreviews.length > 0 && (
                  <div className="mt-2">
                    <Button onClick={goToDescribeStep} className="w-full">
                      Continue with {imagePreviews.length} image
            ...
            <div>
              ...
              <div className="mb-4">
                <div className="flex overflow-x-auto gap-2 pb-2 pt-2">
                  {imagePreviews.map((preview, index) => (
                    <div
                      key={index}
                      className="relative min-w-16 w-16 h-16 flex-shrink-0"
                    >
                      <img
                        src={preview}
                        alt={`Preview ${index}`}
              ...
              <div className="mb-4">
                ...
                {description.length > 9 && description.trim().length < 250 && (
                  <p
                    className={`text-sm mt-1 text-right ${description.length > 250 ? "text-red-500" : "text-grey-500"}`}
                  >
          ...
          <div className="py-8 text-center">
            <div className="animate-pulse flex flex-col items-center">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Our AI is calculating measurements from your images
              </p>
            </div>
          </div>
        )}
              ...
              {measurementResult.measurements.additional &&
                measurementResult.measurements.additional.length > 0 && (
                  <div className="border-t border-gray-200 pt-3 mt-3">
                    ...
                    {measurementResult.measurements.additional.map(
              ...
              <div className="mt-4">
                ...
                <div
                  className={cn(
                    ...
                    measurementResult.confidence === "high"
                      ? "bg-green-100 text-green-800"
                      : measurementResult.confidence === "medium"
                        ? "bg-yellow-100 text-yellow-800"
                        : "bg-red-100 text-red-800",
                  )}
                >
                  {measurementResult.confidence.charAt(0).toUpperCase() +
                    measurementResult.confidence.slice(1)}
                </div>
              </div>
            </div>
