...
export const QuestionCard = ({
  ...
  const [selectedInputMethod, setSelectedInputMethod] = useState<string | null>(
    null,
  );
    ...
    if (selectedAnswers.length > 0) {
      ...
      if (question.type === "text_input") {
        ...
        const selectedOption = question.options.find(
          (opt) => opt.value === selectedAnswers[0],
        );
        if (
          selectedOption &&
          (selectedOption.type === "text_input" ||
            selectedOption.type === "number_input")
        ) {
  ...
  const handleOptionClick = async (value: string, option?: any) => {
    if (question.type === "multiple_choice") {
      ...
      if (
        option &&
        (option.type === "text_input" || option.type === "number_input")
      ) {
  ...
  const handleTextInputChange = (
    ...
    const charLimit =
      activeInputOption?.character_limit || question.character_limit;
    ...
    const validationRegex =
      activeInputOption?.validation || question.validation;
    if (validationRegex && value) {
      try {
        const regex = new RegExp(validationRegex);
        if (!regex.test(value)) {
          setError(
            activeInputOption?.validation_message ||
  ...
  const handleMeasurementChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(numValue)) {
      if (
        question?.options?.[0]?.min !== undefined &&
        numValue < question?.options[0]?.min
      ) {
        setError(
          `Value must be at least ${question.options[0].min} ${question.unit || ""}`,
        );
        setNextButtonDisable(true);
        return;
      }
      if (
        question.options?.[0]?.max !== undefined &&
        numValue > question.options[0].max
      ) {
        setError(
          `Value must be at most ${question.options[0].max} ${question.unit || ""}`,
        );
        setNextButtonDisable(true);
        return;
      }
    ...
    if (question.validation) {
      ...
      if (!regex.test(value)) {
        setError(
          question.validation_message || "Please enter a valid measurement",
        );
        setNextButtonDisable(true);
        return;
      }
  ...
  const renderTextInput = (inputOption = null) => {
    ...
    const characterLimit = opt.character_limit
      ? parseInt(opt.character_limit)
      : undefined;
      ...
      >
        <div className="flex flex-col mb-8">
          <div className="flex items-start justify-between">
            ...
            {/* Video guide link */}
            {question.video_guide &&
              renderVideoGuideButton(question.video_guide)}
          </div>
        ...
        {question.type === "camera_measurement" ? (
          <div className="space-y-6 mb-12 px-4 md:px-0">
            ...
            <div className="space-y-6">
              ...
              {options.find(
                (opt) =>
                  opt.type === "number_input" || opt.type === "text_input",
              ) && (
                <div className="space-y-3">
                        ...
                        <div className="relative group">
                          <NumberLabelInput
                            label={
                              ...
                              `Enter ${question.unit ? `in ${question.unit}` : ""}`
                            }
                            NumberInputProp={
                              <NumberInput
              ...
              {options.find((opt) => opt.type === "camera_measurement") && (
                <div>
                  {(() => {
                    const cameraOption = options.find(
                      (opt) => opt.type === "camera_measurement",
                    );
                    return cameraOption ? (
                      <div className="space-y-3">
                        <Button
                        ...
                        >
                          ...
                          <span className="font-medium">
                            {cameraOption.label || "Camera measurement"}
                          </span>
                        </Button>
            ...
            <div className="space-y-3">
                ...
                <div className="relative group">
                  <NumberLabelInput
                    label={
                      ...
                      `Enter measurement${question.unit ? ` (${question.unit})` : ""}`
                    }
                    NumberInputProp={
                      <NumberInput
              ...
              {question.camera_option && (
                ...
                >
                  ...
                  <span>
                    {question.camera_prompt || "Take photo to measure"}
                  </span>
                </Button>
              )}
            </div>
          ...
          >
            {options.map((option) => (
              <QuestionOption
                ...
                onInputChange={(value) =>
                  handleOptionInputChange(option.value, value)
                }
