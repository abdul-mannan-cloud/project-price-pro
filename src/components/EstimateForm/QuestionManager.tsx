...
export const QuestionManager = ({
  ...
  useEffect(() => {
    if (
      currentQuestion &&
      !questionHistory.some((q) => q.id === currentQuestion.id)
    ) {
  ...
  const handleNext = () => {
    ...
    if (currentQuestionIndex < questionHistory.length - 1) {
      ...
      const currentAnswer =
        currentSetAnswers[displayQuestion.id]?.answers || [];
  ...
  const isLastQuestion =
    currentStage === totalStages && !hasFollowUpQuestion && isLatestQuestion;
    ...
    <div className="space-y-6">
      <QuestionCard
        ...
        onNext={
          ...
          displayQuestion.type === "camera_measurement"
