...
export const EstimateTotals = ({
  ...
  return (
    <div
      className={cn(
        "mt-4 sm:mt-6 md:mt-8 pt-3 sm:pt-4 md:pt-6 border-t space-y-2 sm:space-y-4",
      )}
    >
      <div className="flex justify-between items-center">
        <p className={cn(styles.text, isMobile ? "text-xs" : "text-sm")}>
          Subtotal
        </p>
        {isEstimateReady ? (
      ...
      <div className="flex justify-between items-center pt-2 sm:pt-4 border-t">
        <p className={cn(styles.title, isMobile ? "text-sm" : "!text-xl")}>
          Total Estimate
        </p>
        {isEstimateReady ? (
          <p
            className={cn(
              styles.total,
              isMobile ? "text-base font-bold" : "text-xl font-bold",
            )}
          >
            {formatCurrency(totalCost * 1.085)}
          </p>
