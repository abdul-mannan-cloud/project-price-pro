...
const EstimateLockBanner = ({ isLocked, onUnlock, className = "" }) => {
  ...
  return (
    <div
      className={`bg-amber-50 border-l-4 border-amber-500 p-4 mb-4 flex justify-between items-center ${className}`}
    >
...
export const LeadDetailsDialog = ({
  ...
  const [view, setView] = useState<"estimate" | "questions" | "history">(
    "estimate",
  );
...
+ */
  const refreshEstimate = async (leadId: string) => {
    ...
    try {
      ...
      const { data: contractorRow, error: contractorFetchError } =
        await supabase
          .from("contractors")
  ...
  const [showContractorSignatureDialog, setShowContractorSignatureDialog] =
    useState(false);
 ...
 `,
  ...
  useEffect(() => {
    if (initialLead) {
      ...
      if (initialLead.id && !leadIdFromUrl) {
        navigate(`${location.pathname}?leadId=${initialLead.id}`, {
          replace: true,
        });
      }
    }
  ...
  const [isEstimateLocked, setIsEstimateLocked] = useState(
    !!initialLead?.client_signature,
  );
  ...
  const handleCancelLead = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Could not cancel lead.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleArchiveLead = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Could not archive lead.",
        variant: "destructive",
      });
    } finally {
      setIsCancelling(false);
  ...
  const handleUnlockEstimate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "You can now edit this estimate. The client will need to sign again.",
      });
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleLockEstimate = () => {
    ...
    toast({
      ...
      description:
        "The estimate is locked again—you’ll need to unlock to edit.",
    });
  };
  ...
  const handleSaveEstimate = async () => {
    ...
    if (!validateEstimate()) {
      toast({
        ...
        description:
          "Please fill out every title, description, and set a unit price > 0 before saving.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleDuplicateLead = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        navigate(`${location.pathname}?leadId=${data[0].id}`, {
          replace: true,
        });
        handleCloseDialog();
      }
  ...
  const handleSendEstimate = async () => {
    ...
    const hasSelectedOption =
      sendOptions.email || sendOptions.sms || sendOptions.copyLink;
      ...
      <Dialog open={open} onOpenChange={handleCloseDialog}>
        <DialogContent className="max-w-full h-[100vh] p-0 m-0">
          ...
          <div className="flex flex-col h-full relative overflow-hidden pb-20">
            ...
            <div
              className={`flex-1 overflow-y-auto ${isMobile ? "p-0" : "p-6"}`}
            >
                  ...
                  <>
                    ...
                    <div className="mt-4">
                      <EstimateDisplay
                        groups={
                          isEditing
                            ? editedEstimate?.groups || []
                            : lead.estimate_data?.groups || []
                        }
                        totalCost={
                          isEditing
                            ? editedEstimate?.totalCost || 0
            ...
            <div className="flex items-center justify-between space-x-2">
              ...
              {sendOptions.email ? (
                ...
                <span
                  ...
                  onClick={() =>
                    setSendOptions((prev) => ({ ...prev, email: true }))
                  }
                >
                  Add email
                </span>
            ...
            <div className="flex items-center justify-between space-x-2">
              ...
              {sendOptions.sms ? (
                ...
                <span
                  className="text-sm text-blue-600 cursor-pointer"
                  onClick={() =>
                    setSendOptions((prev) => ({ ...prev, sms: true }))
                  }
                >
