...
const CubeLoader = () => {
    ...
    return (
      <div key={`h${height}Container`} className="relative">
        {dimensions.w.map((width) => (
          dimensions.l.map((length) => (
            renderCube(height, width, length)
          ))
        ))}
      </div>
    );
  ...
  return (
    <div className="flex flex-col justify-center items-center m-0 p-0 h-[80vh] w-full" style={{ background: colors.background }}>
      {generateKeyframes()}
