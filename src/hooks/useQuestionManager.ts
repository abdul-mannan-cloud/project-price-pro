...
export const useQuestionManager = (
  ...
  const analyzeQuestionWeights = useCallback(() => {
    ...
    questionSequence.forEach((question) => {
      ...
      if (!question || !question.options) {
        console.warn(
          `Question missing or has no options: ${question?.id || "unknown"}`,
        );
        return;
      }
  ...
  const loadCurrentQuestionSet = () => {
    ...
    try {
      ...
      const sortedQuestions = currentSet.questions.sort(
        (a, b) => a.order - b.order,
      );
  ...
  const moveToNextQuestionSet = () => {
    console.log(
      "Moving to next question set, current index:",
      currentSetIndex,
      "total sets:",
      questionSets.length,
    );
  ...
  const handleSingleChoiceNavigation = (
  ...
  const handleAnswer = async (questionId: string, selectedValues: string[]) => {
    ...
    if (
      (currentQuestion.type === "measurement_input" ||
        currentQuestion.type === "camera_measurement") &&
  ...
  const handleInputTypeNext = (questionType: string) => {
    ...
    console.log(
      `Handling ${questionType} next for question:`,
      currentQuestion.id,
    );
    ...
    const selectedValue =
      currentSetAnswers[currentQuestion.id]?.answers[0] || null;
  ...
  const handleCameraMeasurementNext = () => {
    ...
    console.log(
      "Handling camera measurement next for question:",
      currentQuestion.id,
    );
  ...
  const handleMultipleChoiceNext = async () => {
    ...
    if (!currentQuestion.options || !Array.isArray(currentQuestion.options)) {
      console.warn(
        `Question ${currentQuestion.id} has no options array in handleMultipleChoiceNext`,
      );
      moveToNextQuestionSet();
      return;
    }
    ...
    const nextQuestions = selectedValues.reduce(
  ...
  const handleComplete = async () => {
    ...
    try {
      const answersForDb = Object.entries(answers).reduce(
  ...
  return {
    ...
    hasFollowUpQuestion:
      ...
      (currentQuestionId &&
        questionSequence.find((q) => q.id === currentQuestionId)?.type ===
          "multiple_choice" &&
        questionSequence
          .find((q) => q.id === currentQuestionId)
          ?.options?.some((opt) => opt?.next)),
